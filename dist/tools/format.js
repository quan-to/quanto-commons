...
const format = (str, mask) => {
    ...
    if (validation_1.undefinedOrNull(mask)) {
        return data;
    }
...
String.prototype.removeDiactrics = function removeDiactricsStr() {
    return removeDiactrics_1.default(this.toString());
};
/**
...
Number.prototype.toMoney = function toMoney(decimals, decimalSeparator, separator) {
    ...
    const d = validation_1.undefinedOrNull(decimalSeparator) ? ',' : decimalSeparator;
    const t = validation_1.undefinedOrNull(separator) ? '.' : separator;
    const s = this < 0 ? '-' : '';
